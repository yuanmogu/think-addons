...
class Service extends \think\Service
{
    ...
    private function autoload()
    {
        ...
        foreach (glob($this->getAddonsPath() . '*/*.php') as $addons_file) {
			...
			// 判断插件是否安装到数据库，已安装的插件才生效
			$installed = check_addons_value($name, 'status');
            ...
            if (strtolower($info['filename']) === $name) {
                ...
                foreach ($hooks as $hook) {
					...
					//排除 install 和 uninstall
					//if(in_array($hook,['install','uninstall'])) continue;

					// 只取后缀是Hook的事件
                    if (strlen($hook) < 4 || substr($hook, -4) != 'Hook') continue;
